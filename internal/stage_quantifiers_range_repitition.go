...
func testQuantifierRangeRepetition(stageHarness *test_case_harness.TestCaseHarness) error {
	...
	testCaseCollection := test_cases.StdinTestCaseCollection{
		...
		},
		{
			...
			Input:            fmt.Sprintf("%s v%d_%d_%d stable %s installed", vegetable[:3], random.RandomInt(1, 9), random.RandomInt(0, 20), random.RandomInt(0, 50), fruit1),
			ExpectedExitCode: 0,
		},
		{
			Pattern:          fmt.Sprintf(`^[a-z]{2,5} v\d{1,2}_\d{1,2}_\d{1,2} (stable|beta) %s (installed|available)$`, fruit1),
			Input:            fmt.Sprintf("%s v%d_%d_ beta %s available", vegetable[:3], random.RandomInt(1, 9), random.RandomInt(0, 20), fruit1),
			ExpectedExitCode: 1,
		},
	}
